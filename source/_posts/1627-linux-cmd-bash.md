...
| -     **`ll | more`**                    | 将ll获得的数据传给more进一步处理                      |
| `cut` `grep` `split`                     | 截取分割, 通常配合 `|`使用                         |
| `sort` `wc` `uniq`                       | 排序命令, 通常配合 `|`使用                         |
| `tr` `col` `join` `paste` `expand`       | 字符转换, 通常配合 `|`使用                         |
| `cmd1;cmd2; cmd3`                        | 可以一行写多个指令                                |
| `cmd1&&cmd2`                             | cmd1执行正确($?为0),则继续执行cmd2, 否则不执行          |
| `cmd1\|\|cmd2`                           | cmd1执行错误($?为!0),则继续执行cmd2, 否则不执行         |
| -     **`ls /tmp/test && echo "exist" || echo "nofile"`** | `/tmp/test`存在显示exist,否则nofile            |
|                                          |                                          |
